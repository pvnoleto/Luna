...
# -*- coding: utf-8 -*-
"""
Script para Aplicar Melhorias Manualmente

Aplica melhorias de uma prioridade específica de forma controlada,
validando cada aplicação antes de prosseguir.

Uso:
    python scripts/aplicar_melhorias.py [--prioridade P] [--dry-run] [--max N]

Opções:
    --prioridade P: Prioridade das melhorias a processar (1-10, padrão: 3)
...
class AplicadorMelhorias:
    ...
    def __init__(self, prioridade: int = 3, dry_run: bool = False, max_melhorias: int = None):
        self.prioridade = prioridade
        self.dry_run = dry_run
        self.max_melhorias = max_melhorias
        self.fila_path = Path("Luna/.melhorias/fila_melhorias.json")
        self.log_path = Path(f"LOGS_EXECUCAO/aplicacao_p{prioridade}.log")
        self.resultados_path = Path(f"LOGS_EXECUCAO/resultados_p{prioridade}.json")
    ...
    def filtrar_prioridade(self, melhorias: List[Dict]) -> List[Dict]:
        """Filtra melhorias pela prioridade especificada."""
        filtradas = [
            m for m in melhorias
            if m.get('prioridade', 0) == self.prioridade
        ]
...
"""
    ...
    def executar(self):
        ...
        self.log("="*80)
        self.log(f"APLICACAO MANUAL DE MELHORIAS P{self.prioridade}")
        self.log("="*80)
        ...
        melhorias_filtradas = self.filtrar_prioridade(melhorias)
        self.log(f"   Melhorias P{self.prioridade}: {len(melhorias_filtradas)}")
        ...
        if self.max_melhorias:
            melhorias_filtradas = melhorias_filtradas[:self.max_melhorias]
        ...
        if self.dry_run:
            ...
            for i, m in enumerate(melhorias_filtradas, 1):
                self.log(f"   {i}. P{m['prioridade']} - {m['tipo']} - {m['alvo']}")
            return

        # Processar melhorias
        self.log(f"[>>] Processando {len(melhorias_filtradas)} melhorias...")
        self.log("")

        for i, melhoria in enumerate(melhorias_filtradas, 1):
            melhoria_id = melhoria.get('id', f"melhoria_{i}")

            # Pular se já processada
            if melhoria_id in processadas:
                self.log(f"[SKIP] [{i}/{len(melhorias_filtradas)}] Pulando (ja processada): {melhoria_id}")
                self.puladas += 1
                continue

            self.log("-"*80)
            self.log(f"[{i}/{len(melhorias_filtradas)}] Processando melhoria:")
            ...
            self.log(f"   Progresso: {self.total_processadas}/{len(melhorias_filtradas)} | Taxa sucesso: {taxa_sucesso:.1f}%")
            self.log("")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Aplicar melhorias manualmente por prioridade")
    parser.add_argument("--prioridade", "-p", type=int, default=3, help="Prioridade das melhorias (1-10, padrão: 3)")
